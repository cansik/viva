...
from viva.data.augmentations.RandomSizeAugmentation import RandomSizeAugmentation
...
BLOCK_STRATEGY_NETWORKS: Dict[str, NetworkConfig] = {
    ...
    "tcn-i-f": NetworkConfig(
        lambda _: ImprovedTCNLandmarkClassifier(INPUT_SIZE_FEATURES_148, use_focal_loss=True)
    ),
    "mlp": NetworkConfig(
...
class BlockStrategy(BaseTrainStrategy[BlockStrategyOptions]):
    ...
    def train_dataset_type(self) -> Union[Type[FaceLandmarkDataset], Callable[..., FaceLandmarkDataset]]:
        ...
        if self._options.size_augmentation:
            augmentations.insert(0, RandomSizeAugmentation(1, 5))

        if self._options.noise_augmentation:
            augmentations.insert(0, RandomNoiseAugmentation(0.05))
